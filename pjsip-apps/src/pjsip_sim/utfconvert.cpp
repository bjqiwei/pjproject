...
{
    log4cplus::Logger log = log4cplus::Logger::getInstance("utf16le_to_utf8");
	if (u16str.empty()) { return std::string(); }
	...
	for (std::u16string::size_type i = 0; i < len; ++i) {
		...
		if (u16char < 0x0080) {
            ...
            //LOG4CPLUS_INFO(log, "1 utf8 " << u8str);
			continue;
		}
		// 2 字节能表示部分
		if (u16char >= 0x0080 && u16char <= 0x07FF) {
            ...
            //LOG4CPLUS_INFO(log, "2 utf8 " << u8str);
			continue;
		}
		// 代理项对部分(4字节表示)
		if (u16char >= 0xD800 && u16char <= 0xDBFF) {
            ...
            //LOG4CPLUS_INFO(log, "4 utf8 " << u8str);
			continue;
		}
		// 3 字节表示部分
		{
			...
			u8str.push_back((char)((u16char & 0x3F) | 0x80));
            //LOG4CPLUS_INFO(log, "3 utf8 " << u8str);
			continue;
		}
	}
...
{
    log4cplus::Logger log = log4cplus::Logger::getInstance("ucs2_to_utf8");
    std::u16string utf16;
    std::string ucs2Text = ucs2;
    while (ucs2Text.size() >= 4) {
        uint16_t u16;
        std::stringstream oss;
        oss << std::hex;
        oss << ucs2Text.substr(0, 4);
        //LOG4CPLUS_INFO(log, "UCS2 字符 " << oss.str());
        oss >> u16;
        utf16.push_back(u16);
        ucs2Text.erase(0, 4);
    }
